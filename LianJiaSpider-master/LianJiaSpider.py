...
regions=["dongcheng","xicheng","chaoyang","haidian","fengtai","shijingshan","tongzho","changping","daxing","yizhuangkaifaq","shunyi","fangshan","mentougo","pinggu","huairo","miyun","yanqing","yanjiao"]
...
def xiaoqu_spider(db_xq,url_page=u"http://bj.lianjia.com/xiaoqu/pg1rs%E6%98%8C%E5%B9%B3/",page=1,region='dongcheng'):
    """
    爬取页面链接中的小区信息
    """
    try:
        ...
        print(e)
        exit(-1)
    # with open(region+'_'+ str(page)+'.html','wb') as f:
        # f.write(soup.encode('utf-8'))
    xiaoqu_list = soup.findAll('div',{'class':'info'})
    for xq in xiaoqu_list:
        info_dict = {}
        info_dict.update({u'小区名称':xq.find('div',{"class":'title'}).find('a').string})

       
        children = xq.find('div',{"class":'positionInfo'}).children
        for child in children:
            if child.strip('') == '':
                print('none')
            # elif child.find('span',{"class":'positionIcon'}):
            #     pass
            elif child.find('a',{"class":'district'}):
                info_dict.update({u'大区域':child.find('a',{"class":'district'}).string})
            elif child.find('a',{"class":'bizcircle'}):
                info_dict.update({u'小区域':child.find('a',{"class":'bizcircle'}).string})
            else:
                attrs = child.strip('').split('/')
                print(attrs)
                # info_dict.update({u'小区域':child.string})

        sys.exit()
        info_dict.update({u'小区户型':xq.find('div',{"class":'positionInfo'}).find('a').string})
        # info_dict.update({u'建造时间':xq.find('div',{"class":'positionInfo'}).find('a').string})

        # info_dict.update({u'小区名称':xq.find('tagList').text})
        # content = xq.find('div',{'class':'positionInfo'}).decode('utf-8')
        # print( content)
        # sys.exit()
        # patern = re.compile(r".+>(.+)</a>.+>(.+)</a>")
        # info = patern.findall(content)
        # print(info)
        # sys.exit()
        # if info:
        #     info=info.groups()
...
def do_xiaoqu_spider(db_xq,region="changping"):
    ...
    url = "http://bj.lianjia.com/xiaoqu/"+region+"/cro21/"
    try:
    ...
    # print(total_pages),小区策略修改
    threads=[]
    for i in range(total_pages):
        url_page=u"http://bj.lianjia.com/xiaoqu/%s/pg%dcro21/" % (region,i+1)
        t=threading.Thread(target=xiaoqu_spider,args=(db_xq,url_page,i+1,region))
        threads.append(t)
    #启动
    for t in threads:
        t.start()
...
def chengjiao_spider(db_cj,url_page=u"http://bj.lianjia.com/chengjiao/pg1rs%E5%86%A0%E5%BA%AD%E5%9B%AD"):
    ...
    for cj in cj_list:
        ...
        content=cj.find('div',{'class':'con'}).renderContents().strip()
